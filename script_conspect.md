...
* Неупорядоченный, key-value (dict) `{1:’Январь’, 2:’Февраль’}` - в ручную задаются индексы массива, при этом они могут
быть не только цифрами `{'январь':1, 'февраль':2}`
...
`sys.argv` - Возвращает список параметров, переданных скрипту
